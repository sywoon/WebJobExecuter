...
function compressPNGByConfig(filePath) {
    var cfgFile = new File(filePath);
    cfgFile.open("r");
    
    while (!cfgFile.eof) {
        var line = cfgFile.readln();
        if (line.slice(-4).toLowerCase() == ".png") {
            var img = new File(line);
            if(img) {
                compressPNGFile(img);
            }
        }
...
// alert(filePath);
var cfgFile = new File(rootFolder.fsName + "/cfg.txt");
cfgFile.open("r");
var line = cfgFile.readln();
cfgFile.close();
...
//mode path
// 1,c:/temp/
// mode 1:folder 2:one image  3:config list
...
if (mode == "1") {
    ...
    var configFilePath = params[1];
    compressPNGByConfig(configFilePath);
}
